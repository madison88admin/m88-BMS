...
const RequestForm = () => {
  ...
  const handleSubmit = async (e: any) => {
    ...
    // Check if over budget and show confirmation (skip for reimbursements and cash advances)
    if (budgetImpact?.isOverBudget && requestType !== 'reimbursement' && requestType !== 'cash_advance' && !showOverBudgetConfirm) {
                ...
                <div className={`rounded-xl p-4 border ${(toNumber(selectedCategoryBudget.remaining_amount) >= totalAmount || requestType === 'reimbursement' || requestType === 'cash_advance') ? 'bg-green-500/10 border-green-500/20' : 'bg-red-500/10 border-red-500/20'}`}>
                  <div className="grid grid-cols-2 gap-4 text-sm">
                    ...
                    <div>
                      ...
                      <p className={`mt-1 font-semibold ${(toNumber(selectedCategoryBudget.remaining_amount) >= totalAmount || requestType === 'reimbursement' || requestType === 'cash_advance') ? 'text-green-600' : 'text-red-600'}`}>
                  ...
                  {toNumber(selectedCategoryBudget.remaining_amount) < totalAmount && requestType !== 'reimbursement' && requestType !== 'cash_advance' && (
