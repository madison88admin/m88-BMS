...
const getExpenseList = () => {
  return OFFICIAL_EXPENSE_LIST.map(item => ({
    ...item,
    displayName: `${item.code} | ${item.itemName}`,
    eligibility: {
      cashAdvance: item.canCA,
      reimbursement: item.canRE
    }
  }));
};

const getExpensesByDepartment = (departmentName) => {
  const userDept = departmentName.trim().toLowerCase();
  return OFFICIAL_EXPENSE_LIST.filter(item => {
    const allowedDepts = (Array.isArray(item.dept) ? item.dept : [item.dept]).map(d => d.toLowerCase());
    return allowedDepts.includes('all dept') || allowedDepts.some(d => userDept.includes(d) || d.includes(userDept));
  });
};

const getExpensesByCategory = (category) => {
  return OFFICIAL_EXPENSE_LIST.filter(item => item.category === category);
};

const searchExpenses = (query, departmentName = null) => {
  const searchQuery = query.toLowerCase();
  let results = OFFICIAL_EXPENSE_LIST.filter(item => 
